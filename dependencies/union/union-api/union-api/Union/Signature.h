...
#define SIGNATURE_OF(what) Union::Signature(what, #what)
  ...
  private:
    StringANSI NormalizeReturnType(const StringANSI& typeName) const;
    StringANSI NormalizeArgType(const StringANSI& typeName) const;
  };
  ...
  inline Signature::Signature( T detailsPtr, const StringANSI& baseStorageClass ) : Constancy(false) {
    ...
    static StringANSI patternParseStorageClass = "&((\\w+)::)(\\w+)";
  ...
  inline bool Signature::IsSimilarWith( Signature* sig, bool checkNames ) const {
    if( NormalizeReturnType(TypeName) != NormalizeReturnType(sig->TypeName) )
      return false;
    ...
    for( auto i = 0; i < ArgumentsTypeNames.GetCount(); i++ )
      if( NormalizeArgType(ArgumentsTypeNames[i]) != NormalizeArgType(sig->ArgumentsTypeNames[i]) )
        return false;
  ...
  inline StringANSI Signature::NormalizeReturnType(const StringANSI& type) const
  {
    std::string result{type};

    // Remove qualifiers and keywords
    static const std::regex qualifiers(R"(\b(const|struct|class|enum)\b)");
    result = std::regex_replace(result, qualifiers, "");
  ...
  inline StringANSI Signature::NormalizeArgType(const StringANSI& type) const {
    std::string result{type};
    ...
    // Replace & and * with a common symbol (e.g., @)
